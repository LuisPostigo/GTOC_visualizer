...
const useSolutions = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zustand$2f$esm$2f$react$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["create"])((set, get)=>({
        solutions: [],
        visible: {},
        /* === Load all persisted solutions from /api/solutions === */ loadPersisted: async ()=>{
            try {
                const res = await fetch("/api/solutions");
                if (!res.ok) throw new Error("Failed to load persisted solutions");
                const data = await res.json();
                const sols = data.solutions ?? [];
                const visibility = Object.fromEntries(sols.map((s)=>[
                        s.id,
                        true
                    ]));
        ...
        /* === Import new file → persist to disk → load it locally === */ importSolution: async (file)=>{
            try {
                ...
                const sol = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$components$2f$gtoc$2f$solutions$2f$parseSolutionFile$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["parseSolutionFile"])(file);
                set((state)=>({
                        solutions: [
                            ...state.solutions,
                            sol
        ...
        /* === Delete a solution (both locally + server-side) === */ deleteSolution: async (id)=>{
            try {
        ...
        /* === Toggle visibility of a given solution === */ toggle: (id)=>set((state)=>({
                    visible: {
                        ...state.visible,
                        [id]: !state.visible[id]
                    }
                })),
        /* === Recolor a given solution === */ recolorSolution: (id, newColor)=>set((state)=>{
                console.log(`[useSolutions] recolorSolution → ${id} = ${newColor}`);
                const updated = state.solutions.map((s)=>s.id === id ? {
                        ...s,
                        color: newColor
                    } : s);
                return {
...
//# sourceMappingURL=src_components_gtoc_7aec1b8b._.js.map