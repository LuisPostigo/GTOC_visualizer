...
function ViewerCanvas(props = {}) {
    ...
    const handleStartExport = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "ViewerCanvas.useCallback[handleStartExport]": async (filename, exportRate)=>{
            ...
            setPlaying(false);
            // Compositing canvas for overlaying labels
            const compCanvas = recorder.getCompositingCanvas();
            const compCtx = compCanvas.getContext("2d");
            // Composite WebGL + DOM labels onto offscreen canvas
            const compositeFrame = {
                "ViewerCanvas.useCallback[handleStartExport].compositeFrame": ()=>{
                    ...
                    // drei <Html> renders overlays as positioned divs inside the R3F root
                    // Use getBoundingClientRect to find their screen positions reliably
                    const canvasRect = canvas.getBoundingClientRect();
                    const scaleX = TARGET_W / canvasRect.width;
                    const scaleY = TARGET_H / canvasRect.height;
                    // Find the R3F root container which holds <Html> portals
                    const r3fRoot = canvas.parentElement;
                    if (!r3fRoot) return;
                    // drei creates overlay divs with pointer-events:none that contain the Html content
                    // Query for all visible text elements inside the R3F container (excluding the canvas itself)
                    const allOverlayEls = r3fRoot.querySelectorAll("div");
                    allOverlayEls.forEach({
                        "ViewerCanvas.useCallback[handleStartExport].compositeFrame": (el)=>{
                            // Skip elements that are not visible or are the canvas parent itself
                            const computed = window.getComputedStyle(el);
                            if (computed.display === "none" || computed.visibility === "hidden") return;
                            if (computed.opacity === "0") return;
                            // Only process leaf elements with text content
                            const text = el.childNodes.length === 1 && el.childNodes[0].nodeType === Node.TEXT_NODE ? el.textContent?.trim() : null;
                            if (!text || text.length === 0) return;
                            // Get screen position via getBoundingClientRect
                            const elRect = el.getBoundingClientRect();
                            const centerX = (elRect.left + elRect.width / 2 - canvasRect.left) * scaleX;
                            const centerY = (elRect.top + elRect.height / 2 - canvasRect.top) * scaleY;
                            // Skip if outside canvas bounds
                            if (centerX < 0 || centerX > TARGET_W || centerY < 0 || centerY > TARGET_H) return;
                            ...
                            if (bgColor && bgColor !== "rgba(0, 0, 0, 0)" && bgColor !== "transparent") {
                                ...
                                const w = elRect.width * scaleX + padX * 2;
                                const h = elRect.height * scaleY + padY * 2;
                                const rx = 4 * scaleX;
                                const x = centerX - w / 2;
                                const y = centerY - h / 2;
                                compCtx.beginPath();
                                compCtx.roundRect(x, y, w, h, rx);
                                compCtx.fill();
                            }
                            compCtx.font = `${fontWeight} ${Math.max(14, fontSize)}px sans-serif`;
                            compCtx.fillStyle = color;
                            compCtx.textAlign = "center";
                            compCtx.textBaseline = "middle";
                            compCtx.shadowColor = "rgba(0,0,0,0.8)";
                            compCtx.shadowBlur = 4 * scaleX;
                            compCtx.fillText(text, centerX, centerY);
                            compCtx.restore();
                        }
                    }["ViewerCanvas.useCallback[handleStartExport].compositeFrame"]);
                }
...
//# sourceMappingURL=src_components_gtoc_ca83ed85._.js.map