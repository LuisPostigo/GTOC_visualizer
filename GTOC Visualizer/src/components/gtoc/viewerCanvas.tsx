...
export default function ViewerCanvas(props: ViewerProps = {}) {
  ...
  const handleStartExport = useCallback(async (filename: string, exportRate: number) => {
    ...
    const startJD = keys.length > 0 ? keys[0].jd : JD_EPOCH_0;
    const endJD = keys.length > 0 ? keys[keys.length - 1].jd : JD_EPOCH_0 + 60;
  ...
  const videoLayout = useMemo(() => {
    ...
    const margin = { top: 110, bottom: 150, left: 20, right: 20 };
    ...
    return { w, h, marginTop: margin.top, scale: 1, isScaled: false };
  }, [isMovieMode, windowSize, aspectRatio]);

  return (
    <div className="relative w-full h-screen bg-black flex flex-col overflow-hidden items-center justify-start">
      ...
      <div
        ...
        style={
          isMovieMode && videoLayout && !isPresentationMode && !isExporting
            ? {
              width: videoLayout.w,
